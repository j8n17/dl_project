...
class CustomDataset(Dataset):
    def __init__(self, root_path, data_txt, augment, modality):
        # 데이터셋 초기화
    ...
    def __getitem__(self, idx):
        ...
        frames = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406] * self.clips, std=[0.229, 0.224, 0.225] * self.clips)])(frames)
        frames = self.augment(frames)
        frames = frames.reshape((self.clips, 3) + frames.size()[1:])

        if self.modality == 'rgb_diff':
            frames = self._get_diff(frames)
    ...
    def _get_diff(self, input, keep_rgb=False):
        ...
        for x in reversed(list(range(1, self.clips))):
            new_data[x, :, :, :] = input[x, :, :, :] - input[x - 1, :, :, :]

        return new_data if keep_rgb else new_data[1:, :, :, :]
    
class CustomDataLoader(BaseDataLoader):
    def __init__(self, root_path, data_txt, batch_size, shuffle=True, num_workers=1, mode='train', modality='rgb'):
        ...
        self.root_path = root_path
        self.data_txt = data_txt
        self.dataset = CustomDataset(self.root_path, self.data_txt, augment=aug, modality=modality)
        if mode == 'train':
